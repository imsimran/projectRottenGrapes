...
wms.Source = L.Layer.extend({
    ...
    'refreshOverlay': function() {
        ...
        if (!subLayers) {
            ...
            this._overlay.setParams({'layers': subLayers},false);
    ...
    'getFeatureInfo': function(point, latlng, layers, callback) {
        ...
        function done(result) {
            this.hideWaiting();
			let popupText = `<table class="table"><tr><th>Area(Sq.Km)</th><th>Type</th></tr><tr><td>${parseFloat(JSON.parse(result).features[0].properties.poly_area).toFixed(3)}</td><td>${JSON.parse(result).features[0].properties.type}</td></tr></table>`
            var text =this.parseFeatureInfo(popupText, url);
